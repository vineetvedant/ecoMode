#!/usr/bin/env python3
"""Compare ecoMode fixtures against baseline fixtures."""
...
def main() -> int:
        ...
        rows.append(
            {
                ...
                "baseline": baseline,
                "ecomode": ecomode,
                "eco_vs_baseline": baseline - ecomode,
    ...
    for row in rows:
        print(
            "{id}: normal={normal} baseline={baseline} ecomode={ecomode} "
            "eco_delta={eco_vs_baseline:+d} saved={eco_saved_vs_normal:.1f}% "
            "exact={exact} win={win}".format(**row)
        )

    baseline_saved = pct_saved(totals["normal"], totals["baseline"])
    ecomode_saved = pct_saved(totals["normal"], totals["ecomode"])
    print("")
    print(f"total_normal: {totals['normal']}")
    print(f"total_baseline: {totals['baseline']} ({baseline_saved:.1f}% saved)")
    print(f"total_ecomode: {totals['ecomode']} ({ecomode_saved:.1f}% saved)")
    print(f"eco_token_advantage: {totals['baseline'] - totals['ecomode']}")
    ...
    ok = (
        ...
        and totals["ecomode"] < totals["baseline"]
    )
